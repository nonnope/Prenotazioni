document.addEventListener("DOMContentLoaded", () => {
    ...
    const validareForm = () => {
		...
		if (ritiroDataoraObj - now < 120 * 60 * 1000) {
			mostraNotifica("La data e ora di ritiro devono essere almeno due ore successive all'orario attuale.", "info");
		return false;
		}
		...
		if (ritiroMinuti !== 0 && ritiroMinuti !== 30) {
    		mostraNotifica("L'orario di ritiro deve essere a mezz'ora esatta (es. 8:00, 8:30).", "info");
    	return false;
		}
		...
		try {
			...
			if (!risposta.ok) {
				...
				if (risposta.status === 400) {
					mostraNotifica("Tentativo di modifica di una prenotazione scaduta.", "error");
					return { success: false, error: "Tentativo di modifica di una prenotazione scaduta." };
				}


				mostraNotifica(errorData.error || "Errore sconosciuto durante la modifica della prenotazione.", "error");
	...
	const cancellaPrenotazione = async (bookingCode) => {
		...
		try {
			...
			if (!risposta.ok) {
				...
				if (risposta.status === 400) {
					mostraNotifica("Tentativo di cancellazione di una prenotazione scaduta.", "error");
					return {success: false, error: "Tentativo di cancellazione di una prenotazione scaduta."};
	...
	modificaPulsante.addEventListener("click", async (event) => {
        ...
        try {
            ...
            if (!risposta.ok) {
                if (risposta.status === 404) {
                    mostraNotifica("Prenotazione non trovata. Controlla il codice.", "error");
					return;
				} 
